...
public extension MyLeaderboardAPI {
	struct GameDetailsQuery: GraphApiQuery, ResponseAssociable, Equatable {
		...
		fragment GameDetailsFragment on Game { __typename id name hasScores image } fragment GameDetailsStandingsFragment on GameStandings { __typename scoreStats { __typename best worst average } records { __typename player { __typename id avatar } record { __typename ... PlayerGameRecordFragment } } } fragment PlayerGameRecordFragment on PlayerGameRecord { __typename overallRecord { __typename ... RecordFragment } records { __typename opponent { __typename id avatar } record { __typename ... RecordFragment } } } fragment RecordFragment on Record { __typename wins losses ties isBest isWorst } fragment RecentPlayFragment on Play { __typename id playedOn scores game { __typename image } players { __typename id avatar } winners { __typename id } } query GameDetails($id: ID!) { __typename game(id: $id) { __typename ... GameDetailsFragment standings { __typename ... GameDetailsStandingsFragment } recentPlays(first: 3) { __typename ... RecentPlayFragment } } }
		"""
	}
}
